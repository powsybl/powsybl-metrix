...
The MappingTool provides a mapping configuration from csv timeseries to network.
This configuration allows generating iidm network at specified times. Furthermore,
...
The following statuses are produced by the reduced CI coverage tests
...
The following environments variables must be defined to run metrix properly:
`METRIX_ETC`: location of the `.dic` files for a language region (some of these dictionaries are exported in `etc` directory in the installation directory)
