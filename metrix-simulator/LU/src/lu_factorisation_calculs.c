  ...
  if ( LignePivot >= 0 && ColonnePivot >= 0 ) {
    ...
    if ( Matrice->ExclureLesEchecsMarkowitz == OUI_LU ) {		
      ...
      for ( Ki = Kp ; Ki < Matrice->Rang ; Ki++ ) { 
		...
		/* Debut correction RG du 2/2/18 :
		   On peut croire que la matrice est singuliere alors qu'elle en l'est pas */
		if ( ContexteDeLaFactorisation == LU_SIMPLEXE) {
			if ( LigneRejeteeTemporairementPourPivotage[Ligne] == OUI_LU ) {
				LigneRejeteeTemporairementPourPivotage[Ligne] = NON_LU;
				NbT = LNbTerm[Ligne];
				LU_DeClasserUneLigne( Matrice, Ligne, NbT ); // pour etre bien certain de l'enlever avant de la rajouter 
				LU_ClasserUneLigne( Matrice, Ligne, NbT );
				RetourAuDebut = OUI_LU;
			}
			if ( ColonneRejeteeTemporairementPourPivotage[Colonne] == OUI_LU) {
				ColonneRejeteeTemporairementPourPivotage[Colonne] = NON_LU;
				NbT = CNbTermMatriceActive[Colonne];
				LU_DeClasserUneColonne( Matrice, Colonne, NbT ); // pour etre bien certain de l'enlever avant de la rajouter
				LU_ClasserUneColonne( Matrice, Colonne, NbT );
				RetourAuDebut = OUI_LU;
			}
			continue;
		...
		/* if ( ContexteDeLaFactorisation == LU_SIMPLEXE ) continue; */ // Inutile suite a la modif ci dessus
...
if ( Matrice->ContexteDeLaFactorisation == LU_GENERAL ) {
  ...
  if ( Matrice->MxTermesCreesLuUpdate < 1000 ) Matrice->MxTermesCreesLuUpdate = 1000;
