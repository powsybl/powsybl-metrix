...
# if LIMITATION_LU_UPDATE_EN_FONCTION_DU_NOMBRE_DE_REALLOC == OUI_LU
  if ( Matrice->NombreDeReallocTriangleU > 5 && Matrice->NbTermesCreesLuUpdate > Matrice->NbFillIn ) {
    *SuccesUpdate = NON_LU; 
    return;
  }
# endif
...
if ( Matrice->NbTermesCreesLuUpdate >= Matrice->MxTermesCreesLuUpdate ) { 
	# if VERBOSE_LU
	  printf("LU update refusee car trop de termes crees, LU update numero %d NbTermesCreesLuUpdate %d MxTermesCreesLuUpdate %d\n",
            Matrice->NombreDeLuUpdates,Matrice->NbTermesCreesLuUpdate,Matrice->MxTermesCreesLuUpdate);   
	# endif	
	*SuccesUpdate = NON_LU; 
  return;
}
...
if ( *SuccesUpdate == NON_LU ) {
  /* La raz des vecteurs W et Marqueur est inutile puisqu'on va refactoriser */
  # if VERBOSE_LU
	  printf("LU_Update: refactorisation necessaire car vecteurs H satures\n");
  # endif
...
if ( fabs( ElmDeU[il] ) < SeuilPiv && Matrice->NombreDeLuUpdates > 0 ) {
  # if VERBOSE_LU
    printf("Manque de precision dans la LU update numero %d\n",Matrice->NombreDeLuUpdates);
...
# if TEST_DE_STABILITE == OUI_LU
  ...
  if ( LambdaMax * PlusGrandTermeDeLaLigne / fabs( ElmDeU[il] ) > SEUIL_DE_STABILITE && Matrice->NombreDeLuUpdates > 0 ) {
    # if VERBOSE_LU
      printf("Manque de precision dans la LU update numero %d:\n",Matrice->NombreDeLuUpdates);
