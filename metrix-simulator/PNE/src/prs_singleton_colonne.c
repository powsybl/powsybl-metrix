...
if ( Pne->TypeDeVariableTrav[Var] == ENTIER ) {
  PRS_NettoyerLeGrapheDeConflits( Presolve, Var );
}
...
MargeInfaisabilite = MARGE_INITIALE_VARIABLES_CONTINUES;
if ( MargeInfaisabilite > MARGE_INITIALE_VARIABLES_ENTIERES ) MargeInfaisabilite = MARGE_INITIALE_VARIABLES_ENTIERES;
...
/* On n'a pas pu finir ? */
/* Ci-dessous c'etait une connerie */
