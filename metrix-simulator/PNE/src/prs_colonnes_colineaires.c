...
# define ZERO_COLINEAIRE  1.e-10 /*1.e-12*/
...
# define NOMBRE_MAX_DE_FOIS_EQUIVALENT 5 /*5*/
...
for ( Var = 0 ; Var < NombreDeVariables ; Var++ ) {
  ...
  if ( VariableEquivalente[Var] > NOMBRE_MAX_DE_FOIS_EQUIVALENT ) {
	  Flag[Var] = COLONNE_A_EVITER;
		continue;
	}
    ...
    if ( NbVarDispo <= 0 ) goto FinComparaisons;
      ...
      Var = NumVarDeCnt[il];
		  if ( Flag[Var] == COLONNE_A_EVITER ) continue;
			...
			/*if ( NbTermesUtilesDeVar[Var] <= 1 ) continue;*/ /* Colonne singleton */
...
if ( Pne->ContraintesDeBorneVariable != NULL ) {
  PRS_DesactiverContraintesDeBorneVariable( Pne, Var, Var1 );
}
