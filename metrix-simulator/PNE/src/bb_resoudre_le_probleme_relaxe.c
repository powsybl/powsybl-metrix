...
#define COEFF_POUR_COUT_MEILLEURE_SOLUTION  1.e-3       /*1.e-2*/ /* Sert a determiner un seuil pour la reactivation du calcul des coupes. Rq: actuellement on prend la racine carree de la meilleure solution */
#define VALEUR_MIN_SEUIL                    1.0         /*0.0*/
#define VALEUR_MAX_SEUIL                   /*10000.0*/  PLUS_LINFINI

#define REOPTIMISATION_DU_NOEUD_APRES_COUPES     OUI  

#define DERNIER_ROUND_DE_COUPES_AU_NOEUD_RACINE   30 /*30*/ /*20*/
#define DERNIER_ROUND_DE_COUPES_AUX_AUTRES_NOEUDS 2  /*2*/

# define NB_MAX_NOEUDS_ARRET_COUPES 1000 
 
# define CYCLE_CALCUL_COUPES 100
/*# define CYCLE_CALCUL_COUPES_SI_PAS_DE_SOLUTION_ENTIERE 10*/
...
# define MULTIPLICATEUR_NB_ITERATIONS_COUPES_NOEUD_RACINE 1 /*1*/

# define MIN_ITERATION_COUPES_NOEUD_RACINE 1000 /*1000*/

# define MIN_ROUND_DE_COUPES_AU_NOEUD_RACINE 8  /*8*/

# define VERIFIER_ADMISSIBILITE_SOLUTIONS_ENTIERES OUI
...
{
int i; int YaUneSolution; double ValeurOptimale; double Marge; char CalculerDesCoupes; char NumeroDePasse; double Seuil;
...
int LaSolutionEstAdmissible; char OnARelanceUnCalculSansCoupesDeGomory; char CalculerDesCoupesDeGomorySv;


/* RG 11/2016: On force a NULL car par la suite on fait un test dessus pour savoir si ces vecteurs sont deja alloues */
...
/*if ( Bb->NoeudDeLaMeilleureSolutionEntiere == NULL ) CycleCalculCoupes = CYCLE_CALCUL_COUPES_SI_PAS_DE_SOLUTION_ENTIERE;*/
...
/* Test */
/* Si le noeud a un minorant a moins d'une certaine fraction de la meilleure solution, on active le calcul des coupes */
if ( Bb->NoeudDeLaMeilleureSolutionEntiere != 0 && NoeudCourant != Bb->NoeudRacine && MiseEnServiceDesCoupes == NON ) {
  /*Seuil =  COEFF_POUR_COUT_MEILLEURE_SOLUTION * fabs( Bb->CoutDeLaMeilleureSolutionEntiere );*/
  Seuil = sqrt( fabs( Bb->CoutDeLaMeilleureSolutionEntiere ) );
	if ( Seuil < VALEUR_MIN_SEUIL ) Seuil =  VALEUR_MIN_SEUIL;
	else if ( Seuil > VALEUR_MAX_SEUIL ) Seuil =  VALEUR_MAX_SEUIL;
  if( fabs( NoeudCourant->MinorantPredit - Bb->CoutDeLaMeilleureSolutionEntiere ) < Seuil ) {
    MiseEnServiceDesCoupes = OUI;
...
OnARelanceUnCalculSansCoupesDeGomory = NON;
CalculerDesCoupesDeGomorySv = Bb->CalculerDesCoupesDeGomory;
Bb->CalculerDesCoupesDIntersection = OUI_PNE;;
...
if ( 
  ...
  if ( ReFaireUnCalculSansCalculerDesCoupes == OUI ) { 
    /*CalculerDesCoupes = NON_PNE;*/
		...
		/* On ne neutralise que le calcul des Gomory ou les coupes d'intersection */
    /*
		if ( Pne->YaUneSolutionEntiere == NON_PNE ) Bb->CalculerDesCoupesDeGomory = NON_PNE;
		else Bb->CalculerDesCoupesDIntersection = NON_PNE;
		*/
    Bb->CalculerDesCoupesDeGomory = NON_PNE;		

    ReFaireUnCalculSansCalculerDesCoupes = NON;
		OnARelanceUnCalculSansCoupesDeGomory = OUI;
	  if ( Bb->AffichageDesTraces == OUI_PNE && Bb->NoeudEnExamen == Bb->NoeudRacine ) {			
		  printf("Gomory cuts computation temporarily disabled .. seems to be inaccurate ..\n");
		}		
  }
  ...
  if ( ReFaireUnCalculSansCalculerDesCoupes == NON && 
    ...
    /* RG 11/2016: attention, il se peut que ces vecteurs soient deja alloues */
    if ( ComplementDeLaBaseSv != NULL ) {
		  free( ComplementDeLaBaseSv );
			ComplementDeLaBaseSv = NULL;
		}
    if ( PositionDeLaVariableSv != NULL ) {
  ...
  if ( Bb->NoeudEnExamen == Bb->NoeudRacine ) {  
	...
	Bb->NumeroDePasse = NumeroDePasse;
      ...
      if ( NumeroDePasse > 2 ) {
       ...
       /* 1- Calcul en pourcentage */
        ...
        if ( CalculerDesCoupes == OUI_PNE ) {											
          if ( fabs( Pne->Critere - AncienneValeurDuCritere ) < VALEUR_ABSOLUE_VARIATION_CRITERE && NumeroDePasse >= MIN_ROUND_DE_COUPES_AU_NOEUD_RACINE ) {
        ...
        if ( Bb->NombreDeVariablesEntieresDuProbleme > 0 ) {
          if ( NoeudCourant == Bb->NoeudRacine && NumeroDePasse >= 2 ) {
              ...
              if ( NbIterationSimplexeCoupes > MxIterationsSimplexeCoupesNoeudRacine && NumeroDePasse >= MIN_ROUND_DE_COUPES_AU_NOEUD_RACINE ) {							
	              m = Bb->ToleranceDOptimalite * 0.01 * fabs( Pne->Critere );					
	              if ( m < VALEUR_ABSOLUE_VARIATION_CRITERE ) m = VALEUR_ABSOLUE_VARIATION_CRITERE;						
						    /* On continue si le critere decroit */
                if ( fabs( Pne->Critere - AncienneValeurDuCritere ) > m ) {
  ...
  if ( YaUneSolution == NON && NumeroDePasse > 1 ) {
    /* Cela signifie que les coupes ajoutees on fait diverger le simplexe car elles etaient mal 
       fichues. Si on ne l'a pas deja tente on relance un calcul sans les Gomory */
    /* Ce qui aura pour effet de simplement de liberer les tableaux de coupes dans la pne, mais on n'en 
       stockera pas */
    YaUneSolution   = OUI; 
    YaUneSolutionSv = OUI;
    if( Bb->SortieParDepassementDuCoutMax != OUI && OnARelanceUnCalculSansCoupesDeGomory == NON ) {		  
      /* 10/3/2016
			Si on est a une resolution qui a "echoue" apres l'ajout de coupes, on relance 
      le calcul en interdisant le calcul de certaines coupes si cela n'a pas deja ete fait.
			Sinon on confirme l'absence de solution */			 		
		  ReFaireUnCalculSansCalculerDesCoupes = OUI;
		}		
...
# if VERIFIER_ADMISSIBILITE_SOLUTIONS_ENTIERES == OUI
  if( YaUneSolution == OUI && *SolutionEntiereTrouvee == OUI ) {
    /* On verifie l'admissibilite de la solution entiere. Si des contraintes sont violees on dit que le noeud
		   n'a pas de solution */		
    PNE_VerifierAdmissibiliteDeLaSolutionCourante( Pne, &LaSolutionEstAdmissible );
		if ( LaSolutionEstAdmissible == NON ) {					
      YaUneSolution = NON; 
		}
  }
# endif
...
if ( YaUneSolution == OUI && ChoisirLaVariableAInstancier == 0 ) {
  if ( *SolutionEntiereTrouvee == NON ) {
	  printf("!!!!!!!! Attention Bug dans la partie Branch and Bound (ResoudreLeProblemeRelaxe) ChoisirLaVariableAInstancier = 0 et on va continuer !!!\n");
		/*exit(0);*/
	}
}
...
if ( OnARelanceUnCalculSansCoupesDeGomory == OUI ) {
  /* On repositionne l'indicateur */
  Bb->CalculerDesCoupesDeGomory = CalculerDesCoupesDeGomorySv;
}

if ( Bb->CalculerDesCoupesDeGomory != CalculerDesCoupesDeGomorySv ) {
  printf("Bb->CalculerDesCoupesDeGomory != CalculerDesCoupesDeGomorySv donc exit\n");
	exit(0);
}
