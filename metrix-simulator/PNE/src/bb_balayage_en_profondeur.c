...
# define REINITIALISATION_PERIODIQUE_DU_NOEUD_DE_DEPART NON /*OUI*/
# define NOMBRE_DECHECS_POUR_LA_REINITIALISATION 10
# define INCREMENT_DU_NOMBRE_DECHECS_POUR_LA_REINITIALISATION 10

/*---------------------------------------------------------------------------------------------------------*/

int BB_BalayageEnProfondeur( BB * Bb , NOEUD * NoeudDeDepart , int ProfondeurDuNoeudDeDepart )
{
NOEUD * NoeudsAExplorer[2]; NOEUD * NoeudCourant; NOEUD * NoeudAntecedent; NOEUD * NoeudPereSuivant;
ldiv_t QuotientEtReste; int ProfondeurLimite; char ProfondeurRapide; int ProfondeurMoyenneDesSolutionsEntieres;
int ProfondeurMoyenneDesSolutionsAmeliorantes; double MinorantPredit0; char MinorantPredit0Initialise; 
double MinorantPredit1; char MinorantPredit1Initialise; int IndiceChoisi; char UnIndiceAEteChoisi; char Les2ChoixSontPossibles;
...
int i; PROBLEME_PNE * Pne; double VecteurPourMoyenneMobile[TAILLE_MOYENNE_MOBILE]; char TesterLaDecroissance; int NbTestsMoyenneMobile;
int IndexMoyenneMobile; double SommePourMoyenneMobilePrecedente; double SommePourMoyenneMobile;
char SommePourMoyenneMobilePrecedenteEstInitialisee; int NombreDeCriteresSuccessifsEgaux;
double CritereDeReference;

# if REINITIALISATION_PERIODIQUE_DU_NOEUD_DE_DEPART == OUI
  int NbEchecs; int NbEchecsPourLaReinitialisation;
# endif
...
NombreDeCriteresSuccessifsEgaux = 0;
CritereDeReference = VALEUR_NON_INITIALISEE;
...
# if REINITIALISATION_PERIODIQUE_DU_NOEUD_DE_DEPART == OUI
  NbEchecs = 0;
	NbEchecsPourLaReinitialisation = NOMBRE_DECHECS_POUR_LA_REINITIALISATION;
# endif
...
while ( 1 ) {
  ...
  if ( Bb->TypeDExplorationEnCours == PROFONDEUR_DANS_LARGEUR ) {	
    if ( ProfondeurDuNoeud  > ProfondeurLimite || NbProblemesResolus > NombreMaxDeProblemesProfondeurDansLargeur ) {
		...
		if ( NombreDeCriteresSuccessifsEgaux > Bb->NombreMaxDeCriteresSuccessifsEgauxDansUneRechercheEnProfondeur ) {
		  /*
			printf("Arret de la recherche en profondeur car pas de variation du critere NombreMaxDeCriteresSuccessifsEgauxDansUneRechercheEnProfondeur = %d\n",
			        Bb->NombreMaxDeCriteresSuccessifsEgauxDansUneRechercheEnProfondeur);
      */
			Bb->NombreMaxDeCriteresSuccessifsEgauxDansUneRechercheEnProfondeur += INCREMENT_NOMBRE_MAX_DE_CRITERES_SUCCESSIFS_EGAUX_DANS_UNE_RECHERCHE_EN_PROFONDEUR;
      break;
		}
  }
  ...
  if ( UnIndiceAEteChoisi == OUI ) {
    ...
    if ( SolutionEntiereTrouvee == OUI ) {			
      ...
      if ( Bb->TypeDExplorationEnCours == PROFONDEUR_DANS_LARGEUR && YaUneSolution == OUI ) {
        ...
        /* Pour pouvoir sortir si stagnation du critere */
        if ( CritereDeReference == VALEUR_NON_INITIALISEE ) {
          CritereDeReference = NoeudCourant->MinorantDuCritereAuNoeud;
          NombreDeCriteresSuccessifsEgaux = 0;
				}
				else {
				  /*
			    printf("CritereDeReference - NoeudCourant->MinorantDuCritereAuNoeud = %e nb %d Nbmax %d\n",CritereDeReference - NoeudCourant->MinorantDuCritereAuNoeud,
						      NombreDeCriteresSuccessifsEgaux,Bb->NombreMaxDeCriteresSuccessifsEgauxDansUneRechercheEnProfondeur);
					*/					
				  if ( fabs( CritereDeReference - NoeudCourant->MinorantDuCritereAuNoeud ) < SEUIL_EN_VALEUR_ABSOLUE &&					
               fabs( CritereDeReference - NoeudCourant->MinorantDuCritereAuNoeud ) < fabs( CritereDeReference * SEUIL_EN_VALEUR_RELATIVE ) ) {
            NombreDeCriteresSuccessifsEgaux++;
				  }
					else {
            CritereDeReference = NoeudCourant->MinorantDuCritereAuNoeud;
            NombreDeCriteresSuccessifsEgaux = 0;
  ...
  if ( NoeudPereSuivant == 0 ) {	
    ...
    # if REINITIALISATION_PERIODIQUE_DU_NOEUD_DE_DEPART != OUI
		...
		# else
      /* Test: en recherche en profondeur, on remonte au noeud racine si trops d'echecs */
      if ( Bb->TypeDExplorationEnCours == PROFONDEUR_DANS_LARGEUR ) { 
		    NoeudPereSuivant = BB_RemonterDansRechercheEnProfondeur( NoeudAntecedent, ProfondeurDuNoeudDeDepart );
		  }
		  else {
			  /* On est en recherche d'une solution entiere */
		    NbEchecs++;
			  if ( NbEchecs < NbEchecsPourLaReinitialisation ) {
		      NoeudPereSuivant = BB_RemonterDansRechercheEnProfondeur( NoeudAntecedent, ProfondeurDuNoeudDeDepart );
			  }
		 	  else {
					NbEchecs = 0;
          NbEchecsPourLaReinitialisation += INCREMENT_DU_NOMBRE_DECHECS_POUR_LA_REINITIALISATION;					
          BB_RechercherLeMeilleurMinorant( Bb, RECHERCHER_LE_PLUS_PETIT_GAP_PROPORTIONNEL ); /* ou bien RECHERCHER_LE_PLUS_PETIT */
          if ( Bb->NoeudDuMeilleurMinorant == 0 ) {
		        NoeudPereSuivant = BB_RemonterDansRechercheEnProfondeur( NoeudAntecedent, ProfondeurDuNoeudDeDepart );					
					}
					else {
            NoeudPereSuivant = Bb->NoeudDuMeilleurMinorant;
					}
          if ( Pne->AffichageDesTraces == OUI_PNE ) {
            printf("Restarting DFS from depth %d\n",NoeudPereSuivant->ProfondeurDuNoeud);
					}					
			  }
		  }
		# endif
    ...
    if ( NoeudPereSuivant != 0 ) { 
      ...
      if ( NoeudPereSuivant->NoeudAntecedent != NULL ) NoeudPereSuivant = NoeudPereSuivant->NoeudAntecedent; /* Car on a pu remonter au noeud racine */
    }
    else {
