...
if ( Spx->NbCycles == 0 || Spx->NbCycles >= CYCLE_DAFFICHAGE ) { 
  ...
  # if VERBOSE_SPX
    if ( Spx->NbInfaisabilitesDuales == 1 ) {
      ...
      # ifndef ENLEVER_LES_VARIABLES_XMIN_EGAL_XMAX_DE_LA_LISTE_HORS_BASE			
		    if ( NN > 1 ) {
			    printf("Bug: NbInfaisabilitesDuales %d or on en compte %d\n",Spx->NbInfaisabilitesDuales,NN);
			    exit(0); /* Car c'est un bug */
			  }
      # endif			
    }
  # endif
