...
int AverageG; int AverageI; int AverageK; char Tracer; double EcartPourArretCalculs; double Y;
# if VERBOSE_BB
int cpt; int i; int * pt1 ; char * pt2;
# endif

/*EcartPourArretCalculs = 1.e-6;*/
EcartPourArretCalculs = 0;
...
if ( Bb->NombreDeProblemesResolus > 0 ) {
  ...
  if ( Bb->NombreDeSolutionsEntieresTrouvees > 0 && fabs( Bb->ValeurDuMeilleurMinorant ) > 1.e-9 ) {
	  Y = Bb->CoutDeLaMeilleureSolutionEntiere - Bb->ValeurDuMeilleurMinorant;
    X = Y / (0.01 * fabs( Bb->ValeurDuMeilleurMinorant ));
    X = fabs( X );				
    if ( X <= Bb->ToleranceDOptimalite || fabs( Y ) <= EcartPourArretCalculs ) {		
      Bb->ArreterLesCalculs = OUI;      
      /*Bb->EcartBorneInf = X;*/
      Bb->EcartBorneInf = Bb->ToleranceDOptimalite; /* Pour eviter l'interpretation timeout dans branch and boud calculs */			
    }
      ...
      /* On ne fait ce calcul que si la partie fixe est du meme ordre que ValeurDuMeilleurMinorant */		 
			if ( /*abs( Pne->Z0 ) < fabs( Bb->ValeurDuMeilleurMinorant )*/ fabs( Pne->Z0 ) > 1.e-6 ) { /* Test: on fait toujours ce calcul sauf si Z0 est trop petit */
			  Y = Bb->CoutDeLaMeilleureSolutionEntiere - Bb->ValeurDuMeilleurMinorant;
        X = Y / (0.01 * fabs( Bb->ValeurDuMeilleurMinorant - Pne->Z0 ));
        X = fabs( X );
				/*printf("1- Bb affichage des traces sans Z0 X %e\n",X);*/				
        if ( X <= Bb->ToleranceDOptimalite || fabs( Y ) <= EcartPourArretCalculs ) {		
          Bb->ArreterLesCalculs = OUI;      
          /*Bb->EcartBorneInf = X;*/
          Bb->EcartBorneInf = Bb->ToleranceDOptimalite; /* Pour eviter l'interpretation timeout dans branch and boud calculs */			
    ...
    if ( Bb->NombreDeSolutionsEntieresTrouvees > 0 ) {  
      ...
      if ( fabs( Bb->ValeurDuMeilleurMinorant ) > 1.e-9 ) {
			  Y = Bb->CoutDeLaMeilleureSolutionEntiere - Bb->ValeurDuMeilleurMinorant;
        Bb->EcartBorneInf = Y / (0.01 * fabs( Bb->ValeurDuMeilleurMinorant ));						
	      Bb->EcartBorneInf = fabs( Bb->EcartBorneInf );
        if ( Bb->EcartBorneInf <= Bb->ToleranceDOptimalite ) {				
		    ...
		    if ( Bb->ArreterLesCalculs != OUI ) {
          /* On ne fait ce calcul que si la partie fixe est du meme ordre que ValeurDuMeilleurMinorant */
			    if ( /*fabs( Pne->Z0 ) < 10. * fabs( Bb->ValeurDuMeilleurMinorant*/ fabs( Pne->Z0 ) > 1.e-6 ) { /* Test: on fait toujours ce calcul sauf si Z0 est trop petit */
					  Y = Bb->CoutDeLaMeilleureSolutionEntiere - Bb->ValeurDuMeilleurMinorant;
            X = Y / (0.01 * fabs( Bb->ValeurDuMeilleurMinorant - Pne->Z0 ));						
	          X = fabs( X );
						/*printf("2- Bb affichage des traces sans Z0 X %e\n",X);*/						
            if ( X <= Bb->ToleranceDOptimalite || fabs( Y ) <= EcartPourArretCalculs ) {				
	            Bb->ArreterLesCalculs = OUI;					
              /*Bb->EcartBorneInf = X;*/
              Bb->EcartBorneInf = Bb->ToleranceDOptimalite; /* Pour eviter l'interpretation timeout dans branch and boud calculs */			
