...
for ( VarSpx = 0 ; VarSpx < Spx->NombreDeVariables ; VarSpx++ ) {
  ...
  if ( Spx->XminEntree[VarSpx] == 1. ) {	
    /* C'est qu'on a instancie la variable a 1 */
...
/* Il est preferable de ne pas faire le calcul des gomory en hyper creux. De toutes façons une gomory est
   rarement hyper creuse.
	 Finalement je n'en suis pas si sur: il faudra aussi utiliser l'hyper creux pour aller plus vite.
	 Attention, les seules variables pour lesquelles  on peut avoir Inf=Sup sans etre fixe dans la partie
	 PNE sont les variables entieres. Si cela arrive sur des variables continues alors NBarreR est mal calcule */
