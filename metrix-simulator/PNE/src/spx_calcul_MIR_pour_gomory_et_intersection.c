...
if ( PlusGrandTerme/PlusPetitTerme > RapportMaxDesCoeffs ) {	
	/*
	printf("Refus calcul de G car Rapport %e PlusGrandTerme %e PlusPetitTerme %e RapportMaxDesCoeffs = %e  NombreDeTermes = %d \n",
	        PlusGrandTerme/PlusPetitTerme,PlusGrandTerme,PlusPetitTerme,RapportMaxDesCoeffs,*NombreDeTermes);
  */					
  *NombreDeTermes = 0;
  return;
}
...
if ( PlusGrandTerme > PlusGrandTermeDeLaMatrice ) {
  ...
  /* Ajout 2/9/2016 */
	/*
	if ( RapportCoupe * PlusPetitTerme < 1.e-6 ) {
	  if ( RapportCoupe * PlusPetitTerme < PlusPetitTermeDeLaMatrice ) {			
	    *NombreDeTermes = 0;
		  return;		
		}
	}
...
/* Par ailleurs le seuil violation coupes d'intersection a ete releve de 10-5 a 10-4 */
...
/* A tester au lieu de ci-dessus */
/* On ne prend pas les tolerance des la partie Pne */
/* Pour les Gomory on a mis les tolerances a 0 dans la partie Pne */	
/*
if ( RelaxRhsRel != 0 || RelaxRhsAbs != 0 ) {
  Co = -BGomory;
  Co *= ldexp( 1., 30 );
  Co = ceil( Co );
  Co /= ldexp( 1., 30 );
