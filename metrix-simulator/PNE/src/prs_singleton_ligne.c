...
if ( Pne->NbLignesSingleton >= Pne->NombreDeContraintesTrav ) return; /* Test a faire sur la dimension car nb contraintes change */
  ...
  if ( SensContrainte[Cnt] == '=' ) {	
    ...
    if ( A[ilNonFix] > 0. ) {
	    ...
	    /* X est une borne sup candidate */
		  ...
		  # if TRACES == 1							
        printf("Controle pour Mise a jour de borne sup: BorneSupPourPresolve[%d] = %e nouvelle borne = %e\n",
				        VarNonFix,BorneSupPourPresolve[VarNonFix],(double) X);
      # endif
    ...
    else if ( A[ilNonFix] < 0. ) {
		
		  /* X est une borne inf candidate */
			
		  # if TRACES == 1
        printf("Controle pour Mise a jour de borne inf: BorneInfPourPresolve[%d] = %e nouvelle borne = %e\n",
				        VarNonFix,BorneInfPourPresolve[VarNonFix],(double) X);
      # endif			
