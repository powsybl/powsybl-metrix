    ...
    while ( il < ilMax ) {
      ...
      if ( fabs( A[il] ) > SEUIL_DE_PIVOT ) { 
        ...
        if ( CNbTerm < MinTermesColonne ) {
          if ( VarChoix >= 0 && TypeVarChoisie == NON_BORNEE && TypeVar != NON_BORNEE ) { 
            /* Si une variable non bornee a deja ete choisie elle ne peut etre remplacee 
               que par une variable non bornee car les variables non bornees sont 
               prioritaires */							 						
            goto NextElement; 
          } 
