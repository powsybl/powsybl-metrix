...
| SOLVERCH     | I    | 1      | Choix du solveur pour l'optimisation principale (LP/MIP) : 5 SIRIUS, 6 XPRESS. La valeur 6 nécessite un binaire compilé avec `USE_ORTOOLS` et `USE_XPRESS`, faute de quoi la configuration est rejetée ; Xpress étant chargé dynamiquement par OR-Tools, une licence absente à l'exécution provoque une erreur `ERRSolveurIndisponible` à la première résolution.<br>= solverChoice (5)  |
...
| PERTURBCOST  | R    | 1      | Perturbation déterministe des coûts des variables de groupes et de consommations pour lever la dégénérescence (optima alternatifs solveur-dépendants) : le coût d'indice i est multiplié par (1 + PERTURBCOST·i/N), N étant le nombre de variables perturbées. PERTURBCOST est l'amplitude relative maximale de la perturbation, indépendante de la taille du réseau ; la perturbation croît avec l'indice, les consommations (indices élevés) sont donc davantage perturbées que les groupes. Une valeur trop faible (pas PERTURBCOST/N sous la tolérance du solveur) ne départage plus les optima ; 0 = désactivé. La perturbation ne s'applique qu'à la phase hors réseau (empilement économique et délestage HR) : les coûts de redispatching AR sont réécrits ensuite sans perturbation, les optima alternatifs de la phase AR ne sont donc pas départagés par ce mécanisme.<br>= perturbationCout (0.0) |
